...
class Bird:
    ...
    def __init__(self, num: int, xy: tuple[int, int]):
        ...
        img0 =pg.transform.rotozoom(pg.image.load(f"ex03/fig/{num}.png"),0,2.0) #左向き　二倍
        ...
        self._imgs = {
            ...
            (+1,+1):pg.transform.rotozoom(img1,-45,1.0),#右
        }
    ...
    def change_img(self, num: int, screen: pg.Surface):
        ...
        """
        self._img = pg.transform.rotozoom(pg.image.load(f"ex03/fig/プロジェクト演習/{num}.png"), 0, 2.0)
...
class Explosion:
    ...
    def __init__(self, _rct.center):
        eplo_img0 =pg.transform.rotozoom(pg.image.load(f"ex03/fig/explosion.gif"),0,1.0)
        eplo_img1 =pg.transform.flip(eplo_img0, True, False)


        eplo_imgs = [
            eplo_img0, #爆発の通常gif
            eplo_img1, #爆発の左右反転gif
            pg.transform.rotozoom(eplo_img0,180,1.0),#爆発の上下反転gif
            pg.transform.rotozoom(eplo_img1,180,1.0) #爆発の上下左右反転gif
        ]
...
def main():
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT: #バツがクリックされたら
                return
        ...
        if beam is not None:  # ビームが存在しているとき
            ...
            for i, bomb in enumerate(bombs):
                if beam._rct.colliderect(bomb._rct):
                    beam = None
                    del bombs[i]
                    bird.change_img(6, screen)#爆弾が爆発した瞬間はここ
                    break
        pg.display.update()
        clock.tick(1000)
